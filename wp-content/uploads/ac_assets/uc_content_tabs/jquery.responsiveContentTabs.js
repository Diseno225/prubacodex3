;(function ( $, window, undefined ) {
  ...
  ResponsiveContentTabs.prototype.init = function () {
    ...
    $(window).on('hashchange', function(e) {
      setTimeout(function() {
        ...
        if(oTab) oTab._ignoreHashChange = false; // Reset it
      }, 20);
    });
  ...
  */
  ResponsiveContentTabs.prototype._openTab = function(e, oTab, closeCurrent, stopRotation) {
    ...
    var dataDeviceMode = jQuery('body').attr("data-elementor-device-mode");
    
    // if(objHiddenParentDesktop.length >= 1 && objHiddenParentDesktop.is(':visible') == false && dataDeviceMode == 'desktop')
    //   return(false);
    
    // if(objHiddenParentTablet.length >= 1 && objHiddenParentTablet.is(':visible') == false && dataDeviceMode == 'tablet')
    //   return(false);
    
    // if(objHiddenParentTablet.length >= 1 && objHiddenParentMobile.is(':visible') == false && dataDeviceMode == 'mobile')
    //   return(false);  
    
    var _this = this;
    var scrollOffset;
    // Check if the current tab has to be closed
    if(closeCurrent || _this.options.closeCurrent === true) {
